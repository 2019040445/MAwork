...
class MultiLevelDDI(nn.Module):
    ...
    def forward(self, d1_node, d1_attn_bias, d1_spatial_pos, d1_in_degree, d1_out_degree, d1_edge_input,
        ...
        amde_d1_feature , amde_d2_feature= self.amde(adj_1, nd_1, ed_1,
                               adj_2, nd_2, ed_2,
                               d1, d2, mask_1, mask_2)  #[b,203]



        molor_d1_feature= self.icnn(molor_d1_feature.permute(0, 2, 1)) #[4, 16, 63])
        d1_feature = molor_d1_feature.view(drug1_n_graph, -1) #[4, 1008]
        molor_d2_feature = self.icnn(molor_d2_feature.permute(0, 2, 1))
        d2_feature = molor_d2_feature.view(drug1_n_graph, -1)
        # print(d1_feature.shape,amde_d1_feature.shape)
        d1_feature = torch.cat((d1_feature , amde_d1_feature),dim = 1)
        d2_feature = torch.cat((d2_feature , amde_d2_feature),dim = 1)
        final_fts_sum = d2_feature + d1_feature
...
class MandA():
    ...
    def forward(self, d1_node, d1_attn_bias, d1_spatial_pos, d1_in_degree, d1_out_degree, d1_edge_input,
                d2_node, d2_attn_bias, d2_spatial_pos, d2_in_degree, d2_out_degree, d2_edge_input,
                adj_1, nd_1, ed_1, adj_2, nd_2, ed_2, targets, d1, d2, mask_1, mask_2):

        ## Molormer中torch.cat之上的全部复制
        '''
        Molormer的code
        '''
        # drug1_output = ?
        ...
        drug1_graph_attn_bias[:, :, 1:, 1:] = drug1_graph_attn_bias[:, :, 1:,
                                              1:] + drug1_spatial_pos_bias  # [2, 8, 257, 257]
        ...
        drug1_edge_input = (drug1_edge_input.sum(-2) / (drug1_spatial_pos.float().unsqueeze(-1))).permute(0, 3, 1,
                                                                                                          2)  # 2,8,43,43
        ...
        '''
        AMDE得有一个或多个输出结果，跟Molormer的结果进行拼接cat
        '''
