...
class AMDE(nn.Module):
    ...
    def __init__(self, node_fts_1, edge_fts_1, node_fts_2, edge_fts_2,  out_fts):
        ...
        self.message_size = 25
        self.message_passes = 2
    ...
    def forward(self, adj_1, nd_1, ed_1, adj_2, nd_2, ed_2, de_1, de_2, mask_1,mask_2):
        ...
        node_batch_node_neighbours_1 = torch.zeros(node_batch_size_1, max_node_degree_1, self.node_fts_1).cuda()
        node_batch_edges_1 = torch.zeros(node_batch_size_1, max_node_degree_1, self.edge_fts_1).cuda()

        node_batch_neighbour_neighbour_indices_1 = torch.cat([torch.arange(i) for i in node_degrees_1]).cuda()
        edge_batch_node_batch_indices_1 = torch.cat(
            [i * torch.ones(degree) for i, degree in enumerate(node_degrees_1)]
        ).long().cuda()
        node_batch_node_neighbour_mask_1 = torch.zeros(node_batch_size_1, max_node_degree_1).cuda()
        ...
        node_batch_node_neighbours_2 = torch.zeros(node_batch_size_2, max_node_degree_2, self.node_fts_2).cuda()
        node_batch_edges_2 = torch.zeros(node_batch_size_2, max_node_degree_2, self.edge_fts_2).cuda()
...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        input_ids=input_ids.type_as(b).cuda()
        # print(input_ids.is_cuda)
